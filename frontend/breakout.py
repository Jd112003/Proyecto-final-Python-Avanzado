...
class Screen:
    def __init__(self):
        # Simplificado para debug: Dibujar directo a la ventana, sin escalado
        self.window = pg.display.set_mode((LOGICAL_W, LOGICAL_H))
...
class SFX:
    def __init__(self):
        # Let pygame choose defaults for web compatibility
...
class Game:
    def __init__(self):
        ...
        self.bigfont = pg.font.Font(None, 52)
        self.input_font = pg.font.Font(None, 36)
    ...
    def start_level(self, level_num):
        ...
        self.score = 0 # Reset score on new game start from menu? Or keep? Typical is reset.
    ...
    def update(self, dt):
        if self.state == STATE_MENU:
            # Update simple background animation or logic if needed
            return
        ...
        if all(not b.alive for b in self.bricks):
            ...
            if next_lvl <= 5:
                # Auto-advance to next level logic, OR return to menu? 
                # User asked "ir desbloqueando". Let's auto-advance but with a message or simple transition.
                # For simplicity/arcade style: Auto-advance.
                self.level = next_lvl
                self.lives += 1
                self.bricks = build_level(self.level)
                self.bg_color = get_bg_color(self.level)
    ...
    def draw_menu(self, surf: pg.Surface):
        ...
        for i, rect in enumerate(self.level_buttons):
            ...
            if is_locked:
                # Draw lock icon (simple X or text)
    ...
    def draw(self):
        self.screen.begin(self.bg_color)
        s = self.screen.window
    ...
    def _center_text(self, text, color):
        ...
        self.screen.window.blit(img, (LOGICAL_W//2 - img.get_width()//2, LOGICAL_H//2 - img.get_height()//2))
        ...
        if self.state == STATE_MENU:
            ...
            if e.type == pg.KEYDOWN:
                if e.key == pg.K_ESCAPE or e.key == pg.K_m:
                    self.state = STATE_MENU # Back to menu
                    self.bg_color = BG_MENU
                if e.key == pg.K_SPACE:
                    self.launch_ball()
                if e.key == pg.K_p:
                ...
                if e.key == pg.K_F10:
                    if self.bricks:
                        # Keep one brick and move it close to paddle
                        survivor = self.bricks[0]
                        survivor.rect.centerx = self.paddle.x
                        survivor.rect.bottom = self.paddle.y - 100
